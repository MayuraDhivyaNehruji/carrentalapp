...
function calculateLateFeesForReservation(details, callback){
    let query = "SELECT p.late_fee, p.price, r.return_datetime, estimated_return_datetime FROM reservation as r INNER JOIN price as p on r.vehicle_type_id = p.vehicle_type_id WHERE r.id = " + details.reservation_Id + " AND r.vehicle_id = " + details.vehicle_Id;
    query = query + " AND p.min_range < " + details.booking_hours + ";";
    mysql.executeQuery(query, function (err, result) {
        if (err){
            ...
            if (result){
                if (result.length > 0) {
                    let date1 = new Date(result[0].estimated_return_datetime);
                    let date2 = new Date(result[0].return_datetime);
                    let diff = date2 - date1;
                    let output = Math.round((diff/1000)/60);
                    let totalAmount = 0;
                    if (output < 0){
                        totalAmount = parseInt(result[0].price) * parseInt(details.booking_hours);
                    }else {
                        totalAmount = ((parseInt(result[0].late_fee) * parseInt(output)) / 60) + (parseInt(result[0].price) * parseInt(details.booking_hours));
                    }
