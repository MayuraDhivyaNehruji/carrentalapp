...
function getbookinghistory(userid, callback) {
    let query = "SELECT id, vehicle_id, reservation_datetime, estimated_return_datetime, location, length_of_stay, charge as price, is_returned FROM reservation WHERE user_id=" + userid + " AND is_deleted=0";
    mysql.executeQuery(query, function (err, result) {
        if (err) {
            callback(err, null);
        }
...
function notifyBookingComplete(details, callback) {
    let d = reservation.GetFormattedDate(new Date());
    let query = "UPDATE reservation, vehicle SET reservation.is_returned=1, reservation.is_completed=1, reservation.return_datetime='" + d + "', vehicle.reserved=0 WHERE reservation.id=" + details.reservation_Id + " AND reservation.vehicle_id = vehicle.id ";
                ...
                if (hours > 120) {
                    ...
                    reservation.calculateMembershipFeeForUser(details, function (err, results) {
                        if (err) {
                            ...
                            let d1 = reservation.GetFormattedDate(new Date());
                            let query2 = "UPDATE user_membership SET creation_date='" + d1 + "', expiration_date=DATE_ADD('" + d1 + "', INTERVAL " + type + " MONTH), price=" + results + ", is_payed=0, change_membership_fee=0 WHERE user_id=" + userId + ";";
