            ...
            mysql.executeQuery(query2, function(err,result){
                if(err){
                    callback(err);
                     ...
                     let query3 = "INSERT into price(price_type,price,vehicle_type_id, late_fee, min_range, max_range) VALUES('USD',20,'"+vehicleTypeId+"',10,0,24),('USD',15,'"+vehicleTypeId+"',10,25,48),('USD',10,'"+vehicleTypeId+"',10,49,72)";
                     console.log(query3)
...
function getUserIdForDeletion(callback){
    let query1 = "select * from users where id NOT IN (select DISTINCT(user_id) from reservation where is_completed !=1 and is_returned != 1 and cancellled !=1)  and is_admin !=1 and firstname IS NOT NULL and lastname IS NOT NULL and is_deleted !=1";
    mysql.executeQuery(query1,function(err,result){
        if (err) {
            callback(err);
        } else {
